build:	
	...
	GOOS=linux GOARCH=amd64 godep go build -o build/fugu.v$(VERSION).linux.x86_64
	(cd build && tar -cvzf fugu.v$(VERSION).linux.x86_64.tar.gz fugu.v$(VERSION).linux.x86_64)
	rm build/fugu.v$(VERSION).linux.x86_64

	GOOS=darwin GOARCH=amd64 godep go build -o build/fugu.v$(VERSION).darwin.x86_64
